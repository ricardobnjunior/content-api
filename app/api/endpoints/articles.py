...
def create_article_endpoint(
    ...
    article = create_article(db=db, data=data)
...
def list_articles_endpoint(
    ...
    articles, total = get_articles(db=db, skip=skip, limit=limit)
    return ArticleList(items=articles, total=total)  # type: ignore[arg-type]
...
def update_article_endpoint(
    ...
    article = update_article(db=db, article_id=article_id, data=data)
    if article is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Article not found"
        )
...
def delete_article_endpoint(
    ...
    deleted = delete_article(db=db, article_id=article_id)
    if not deleted:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Article not found"
        )
